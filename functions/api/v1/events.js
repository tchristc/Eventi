...
function parseFilters(searchParams) {
  return {
    locationLat: Number(searchParams.get("location_lat")),
    locationLng: Number(searchParams.get("location_lng")),
    subject: (searchParams.get("subject") || "").trim(),
    category: (searchParams.get("category") || "all").trim(),
    startDate: (searchParams.get("start_date") || "").trim(),
    endDate: (searchParams.get("end_date") || "").trim(),
    radiusMiles: normalizeRadius(searchParams.get("radius_miles"))
  };
}
...
async function getEvents(filters, env) {
  const config = {
...
export async function onRequestGet(context) {
  ...
  const events = await getEvents(filters, context.env);
  ...
  return Response.json({
    ...
    data: {
      filters: {
        ...
        api_sources: (url.searchParams.get("api_sources") || "").split(",").filter(Boolean)
      },
      events
    }
  });
